## remove doublets using scrublet in count matrix format
...
#data = sc.read_10x_mtx(prefixid + "/filtered_feature_bc_matrix", cache = True)
df = pd.read_csv(counttxt, sep = "\t", header = 0, index_col=0)
data = sc.AnnData(df.T)
data.var_names = df.index
data.obs_names = df.columns
